...
public class CharArrayTest {
	...
	@Test//(expected=NumberFormatException.class)
	public void testToDoubleTooManyDigits() {
		for (int nonzeroDigits = 1; nonzeroDigits <= 20; nonzeroDigits++) {
			StringBuilder sb = new StringBuilder("0.0000000000");
			for (int i = 0; i < nonzeroDigits; i++) {
				sb.append(Integer.toString((i % 9) + 1));
			}
			String s = sb.toString();
			_charArray = new CharArray(s);
			log.debug("Trying to convert a digit with {} decimal places, {} decimal digits: {}",
					s.length() - 1, nonzeroDigits, s);
			try {
				double v = _charArray.toDouble();
				log.debug("Parsed double: " + Double.toString(v));
			} catch (Exception e) {
				log.debug("Conversion failed", e);
			}
//			try {
//				double v = Double.parseDouble(s);
//				log.debug("java.lang.Double parsed: " + Double.toString(v));
//			} catch (Exception e) {
//				log.debug("java.lang.Double conversion failed", e);
//			}
		}
	}
