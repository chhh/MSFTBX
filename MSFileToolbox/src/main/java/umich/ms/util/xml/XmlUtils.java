...
public class XmlUtils {
  ...
  /**
   * Reads the run header from the file, locating <b>character (!)</b> positions of {@code
   * <firstTag>} and first {@code <lastTag>} tags. <b></b>
   *
   ...
   * @throws umich.ms.fileio.exceptions.FileParsingException if the tags could not be found or for
   * any IOException
   * @deprecated Try using {@link #locate(List, List, InputStream, long)}. This method uses an XML
   * parser internally, so it's 1) slower than the other method 2) it's reporting of locations is in
      ...
      do {
        ...
        if (loc.getTotalCharsRead() > maxOffset) {
          throw new FileParsingException(String.format(
              "Could not locate tags '%s' and '%s' within first %d characters", tag1, tag2,
              maxOffset));
        }
