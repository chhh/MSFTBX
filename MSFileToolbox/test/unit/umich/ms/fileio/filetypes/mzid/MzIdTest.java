...
public class MzIdTest {
  ...
  public void testParsing() throws Exception {
    ...
    for (Path path : paths) {
      ...
      Assert.assertFalse(String.format("Peptide List was empty for file '%s'", path),
          mzid.getSequenceCollection().getPeptide().isEmpty());
      Assert.assertFalse(String.format("Peptide Evidence List was empty for file '%s'", path),
          mzid.getSequenceCollection().getPeptideEvidence().isEmpty());

      Assert
          .assertFalse(String.format("Spectrum Identification List was empty for file '%s'", path),
              mzid.getDataCollection().getAnalysisData().getSpectrumIdentificationList().isEmpty());
